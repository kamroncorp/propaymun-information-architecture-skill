...
def render_document(data: dict[str, Any]) -> str:
    ...
    domain_section = f"<section><h2>Information domains</h2><div class='domains'>{''.join(domain_rows)}</div></section>"
...
.domains{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px}} .domain{{background:white;border:1px solid var(--line);border-radius:14px;padding:18px}} .domain h3{{margin:0 0 6px}} .domain>p,.domain li p{{color:var(--muted)}} .domain ul{{padding-inline-start:20px}} .domain li span{{display:block;color:var(--muted);font-size:.8rem}}
